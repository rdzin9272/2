if IY_LOADED and not _G.IY_DEBUG == true then
    -- error("RDSCRIPTS HUB is already running!", 0)
    return
end
...
if not game:IsLoaded() then
    ...
    notLoaded.Text = "rdscripts is waiting for the game to load"
    game.Loaded:Wait()
    notLoaded:Destroy()
end
...
Title.TextSize = 18
Title.Text = "RDSCRIPTS FE v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- rdscripts Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the rdscripts support server.'}
...
task.spawn(function()
	...
	minimizeHolder()
	if IsOnMobile then notify("Unstable Device", "On mobile, rdscripts may have issues or features that are not functioning correctly.") end
end)
